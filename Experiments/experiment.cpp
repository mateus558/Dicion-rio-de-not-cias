...
std::vector<duration<float> > time_experiment(const std::string &path, const size_t &n_queries, const size_t &n_terms, const size_t &step, Dictionary *_dict, double *avg_comparisons){
	...
	for(size_t i = 0; i < durations.size(); i++){
		if(i%step == 0){
			plot_data.push_back(std::make_pair(i, (avg/step)*1E3));
			avg = 0.0;
		}
...
int main(int argc, char* argv[]){
	...
	std::clog << "Average query time with 1 term: " << avg_time * 1E3 << " ms" << std::endl;
	std::clog << "Average query time with 2 terms: " << avg_time1 * 1E3 << " ms" << std::endl;
